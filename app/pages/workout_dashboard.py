...
if uploaded_file:
    ...
    df = df.with_columns(
        [
            pl.col("start_time")
            .str.strptime(pl.Datetime, format="%d %b %Y, %H:%M", strict=False)
            .dt.strftime("%A, %B %e, %Y at %I:%M %p"),
            pl.col("end_time")
            .str.strptime(pl.Datetime, format="%d %b %Y, %H:%M", strict=False)
            .dt.strftime("%A, %B %e, %Y at %I:%M %p"),
        ]
    ...
    chart_type = st.selectbox(
        "Select chart type", ["Histogram", "Box Plot", "Line Chart", "Scatter Plot"]
    )
    ...
    if chart_type == "Histogram":
        ...
        df_plot = (
            df.select([x, y] + ([color] if color else [])).drop_nulls().to_pandas()
        )
        fig, ax = plt.subplots()
        if color:
